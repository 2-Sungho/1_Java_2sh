...
//		String input=br.readLine(); // sc.nextLine();
	...
	public void ex2() {
		...
		try {
			...
			System.out.print("입력: ");
			String input=br.readLine(); // IOException(Checked Exception)
	...
	public void ex3() {
		...
		}catch(ArithmeticException e) { // ArithmeticException 산술적예외
			System.out.println("0으로 나눌 수 없습니다.");
		}catch(InputMismatchException e) { // catch문 다중 작성 가능
										   // catch문이 여러개면 위에서부터 순서대로
										   // 발생한 예외를 검사하여
										   // 알맞은 매개변수를 가진 catch에서 처리한다
			//스캐너를 이용한 입력 시 데이터 타입이 올바르지 않으면 발생하는 예외
			// ex) nextInt();를 통한 입력시 정수가 아닌 값을 입력하면 발생
			System.out.println("정수만 입력해주세요.");
		}catch(Exception e) {
			System.out.println("예외가 발생했습니다.");
		}
		finally {
			// finally : catch구문 수행 여부에 관계없이 
			// 무조건 실행하는 구문
