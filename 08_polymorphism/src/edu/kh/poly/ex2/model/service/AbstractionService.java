...
import edu.kh.poly.ex2.model.vo.LSH;
import edu.kh.poly.ex2.model.vo.LSHCalculator;
import edu.kh.poly.ex2.model.vo.Person;

public class AbstractionService {
	
	public void ex1() {
		// 추상 클래스는 정말 객체 생성이 안될까?
		//Animal a=new Animal();
			// Cannot instantiate type Animal 
...
//		f1.breath();
		...
		// 추상 메서드였던 breath() 호출 가능 여부 확인 - 동적 바인딩
		a1.breath();
		a2.breath();
		// 실행 전에는 Animal의 breath()와 연결 된 것으로 보임(정적 바인딩)
		// 실행 중 오버라이딩한 자식의 breath()와 연결됨(동적 바인딩)
		...
		// 강제 오버라이딩한 breath()라는 공통적인 메서드를 가지고있으므로
		// arr[i].breath(); 작성 가능
...
//		KH k1=new KH(); // Cannot instantiate the type KH
		...
		// 클래스/추상 클래스를 상속한 자식 객체는 내부에 부모 객체를 포함하고 있으나
		// 인터페이스를 상속한 자식 객체는 부모 객체를 포함하지 않는다
	...
	public void ex3() {
		Calculator cal=new LSHCalculator();
		...
		System.out.println(cal.mulitple(num1, num2));
		if(num2==0) {
