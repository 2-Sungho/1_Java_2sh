...
//		c2.setWeight();
		//The method setWeight() is undefined for the type Car
...
//		System.out.println(c1.toString());
	...
	public void printCar(Car c) { 
		System.out.println("자동차정보: "+c.toString());
		// 전달 받은 객체가 자식 객체이고
		// toString()을 오버라이딩했다면
		// 동적바인딩을 이용해서 자식 toString() 호출
//	public void printTruck(Truck c) {
//		System.out.println("자동차정보: "+c);
//	}
...
//		((Truck)c3).loading();
		...
		if(c3 instanceof Truck) {
			...
			System.out.println("c3은 Truck 객체가 아닙니다.");
		}
	...
	public void ex4() {
		...
		// 2) 참조하는 객체가 특정타입을 상속 받았는지 검사
		
		Car c=new Spark(); // 업캐스팅
		
		if(c instanceof Car) {
			// c가 참조하는 객체는 Spark이지만
			// 업캐스팅 상태이기 때문에 Car로도 인식됨(다형성)
