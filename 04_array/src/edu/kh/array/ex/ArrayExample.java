...
public class ArrayExample {
	...
	public void ex2() {
		...
		// arr의 값 초기화 확인
		System.out.println("컴파일러가 정말로 0으로 다 초기화해줬을까?");
		...
		// 배열 초기화
		// 1) 인덱스를 이용한 초기화
		arr[0]=100; // arr int[] 참조형 변수가 참조하고있는 배열의 0번 인덱스에 100을 대입
		arr[1]=200;
		arr[2]=300;
		arr[3]=400;
	...
	public void ex3() {
		...
		// 입력받은 키 : 170.5  165.7  184.3  190.2  174.4
		// 평균 : 177.02cm
		...
		// 배열의 값이 잘 대입 되었는지 임시 확인
//		System.out.println(heightArr);
		// 배열의 주소가 출력됨
		// 왜? heightArr 참조 변수에는 배열의 시작 주소가 저장되어 있기 때문
	...
	public void ex4() {
		// ArrayIndexOutOfBoundsException : Array(배열)의 인덱스 범위 초과
		int[] arr=new int[3];
	...
	public void ex6() {
		...
		String[] menuArr= {"김밥","라면","햄버거","순대국","서브웨이","파스타","삼겹살","빵"};
	...
	public void ex7() {
		...
		// 인원수를 입력받아 그 크기만큼의 정수배열을 선언 및 할당
		// 인원수만큼 점수를 입력받아
		// 합계, 평균, 최고점, 최저점을 출력
	...
	public void ex8() {
		...
		// 입력받은 정수가 배열에 존재하면 몇번 인덱스에 존재하는지 출력
		// 단, 없다면 "존재하지 않습니다" 출력
	...
	public void createLottoNumber() {
		...
		for(int i=0;i<lotto.length;i++) {
			...
			// 현재 인덱스(i) 이전의 요소를 순차접근
			for(int x=0;x<i;x++) {
				if(lotto[x]==ran) {
					i--; // i값을 인위적으로 감소시켜 바깥쪽 for문의 증감식(i++)이 실행되었을때
					     // i값이 현재 값을 유지하도록 만든다
	...
	public void ex9() {
		...
		// arr변수에 저장된 배열의 시작 주소를 복사하여 copyArr에 대입(얕은 복사)
		int[] copyArr = arr;
...
//		}
		...
		// 2) System.arraycopy(원본배열,원본 복사 시작 인덱스, 복사배열, 복사배열의 삽입시작 인덱스, 복사길이)
...
//		int[] arr2;
		...
		// arr2가 참조하는 배열이 없을 때
		// 새로운 배열을 생성하여 그 시작 주소를 arr2에 대입한다
		if(arr2==null) {
			arr2=new int[4];
		}
